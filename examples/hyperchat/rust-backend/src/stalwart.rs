//! Stalwart / JMAP archive transport adapter.
//!
//! ## Configuration (environment)
//! - `STALWART_BASE_URL`, `STALWART_USERNAME`, `STALWART_PASSWORD`
...
impl StalwartClient {
    ...
    fn account_id_or_discover(&mut self) -> Result<String, String> {
        if self.api_url.is_none() || self.account_id.is_none() {
            self.discover()?;
        }
        self.account_id
            .clone()
            .ok_or_else(|| "JMAP account id missing".to_string())
    }

    pub fn list_mailboxes_json(&mut self) -> Result<Value, String> {
        let account_id = self.account_id_or_discover()?;
        let resp = self.jmap_call(&[ureq::json!([
            "Mailbox/get",
            {
                "accountId": account_id,
        ...
        let list = resp["methodResponses"]
            .get(0)
            .and_then(|v| v.get(1))
            .and_then(|v| v.get("list"))
            .cloned()
            .unwrap_or_else(|| serde_json::json!([]));
        Ok(serde_json::json!({ "mailboxes": list }))
    }

    pub fn list_emails_json(
        &mut self,
        mailbox_id: String,
        since: Option<String>,
    ) -> Result<Value, String> {
        ...
        if !mailbox_id.trim().is_empty() {
            filter.insert(
                "inMailbox".to_string(),
                Value::String(mailbox_id.trim().to_string()),
            );
        }
        if let Some(s) = since.as_ref().map(|s| s.trim()).filter(|s| !s.is_empty()) {
            filter.insert("after".to_string(), Value::String(s.to_string()));
        }
        let query = if filter.is_empty() {
            ureq::json!({ "accountId": account_id, "sort": [{"property":"receivedAt","isAscending":false}], "limit": 50 })
        } else {
            ureq::json!({ "accountId": account_id, "filter": Value::Object(filter), "sort": [{"property":"receivedAt","isAscending":false}], "limit": 50 })
        };
        let resp = self.jmap_call(&[
            ureq::json!(["Email/query", query, "q-1"]),
            ureq::json!([
                "Email/get",
                {
                    "accountId": account_id,
                    "#ids": { "resultOf": "q-1", "name": "Email/query", "path": "/ids/*" },
